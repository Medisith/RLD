...
CI verde em cada push/PR (`.github/workflows/ci.yml`). Docker é **opcional** — o caminho padrão
é Redis + PHP no host.
...
200 tentativas concorrentes (40 processos × 5) contra capacidade 50 (`scripts/prove_race_condition.php`):
...
também sobre-admite; com `artisan serve` (single-worker) não — o servidor serializa.
...
Dois baldes em sequência (cliente → tenant): abusador barrado no cliente não drena a cota da
organização. **`X-Tenant-Id` não é fronteira de confiança** — só vale se um gateway confiável o
injeta.
...
**Multi-worker** (Fase 10) — harness sem Docker (Linux/macOS/WSL). No Windows nativo não roda;
use WSL ou a evidência na doc. Compose+FPM é opcional.
...
- Multi-worker medido no **harness**, não na stack Laravel+nginx (Compose opcional).
